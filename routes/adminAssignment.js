...
const { getAllAssignments, getAssignmentAnalytics, getAssignmentById, updateAssignment, reassignAssignment, cancelAssignment, createAssignment, getAssignmentByAmmcId } = require('../controllers/adminAssignment');
const { protect, restrictTo } = require('../middlewares/authentication');

router.use(protect);

// User-accessible route for their own policy assignments
router.get('/policy/:ammcId', getAssignmentByAmmcId); // Get assignment by AMMC ID (users can access their own)
